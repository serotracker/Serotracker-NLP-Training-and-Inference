...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            logits = outputs[1]
            loss = torch.nn.functional.cross_entropy(logits, batch[3], ignore_index=-100) + 1 * torch.sum(model.classifier.weight**2)
            # loss = outputs[0]  # model outputs are always tuple in transformers (see doc)

            classifier = list(model.parameters())[-2]
            if step%4 == 0:
              head_histories.append(classifier.clone().detach())
...
def evaluate(args, model, tokenizer, mode, prefix=""):
    ...
    # head_histories = head_histories[-1:]
    stacked_head = torch.stack(head_histories[-50:], 0).float()
    head_mean = torch.mean(stacked_head, 0)
    print(stacked_head.shape)
    # cov = (stacked_head.view(-1, head_mean.view(-1).shape[0]) - head_mean.view(-1)[None, :]).T @ (stacked_head.view(-1, head_mean.view(-1).shape[0]) - head_mean.view(-1)[None, :])
    temp = 120
    cov = temp * torch.mean((stacked_head - head_mean)[:, :, None, :, None] * (stacked_head - head_mean)[:, None, :, None, :], 0)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # hidden = hidden.view(hidden.shape[0], -1) #B x W
            output_mean = hidden @ head_mean.T + model.classifier.bias[None, :]
            c1 = torch.einsum('bd,jkde -> bjke', hidden.float(), cov.float())
            output_covariance = torch.einsum('bjke, be -> bjk', c1.float(), hidden.float())
            first_logit_covariance = 4 * output_covariance[:, 0, 0]
            
            first_logit_noise = torch.empty([output_mean.shape[0], 20], device = 'cuda:0').normal_() * torch.sqrt(first_logit_covariance)[:, None]
            logit_samples = output_mean[:, None, :] + torch.stack([first_logit_noise, torch.zeros_like(first_logit_noise)], -1)
            logits = logit_samples
            print(logit_samples[0])
            # print(logits.shape)

            # output_std = 
            # output_mean = model.named_parameters[] @ head_mean.view(-1, 1)
            

            # print((hidden @ w.T) + b[None, :])
