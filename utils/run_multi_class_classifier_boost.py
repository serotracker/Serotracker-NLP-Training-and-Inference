...
def train(args, train_dataset, model, tokenizer, model_base):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            with torch.no_grad():
              output_base_logits = model_base(**inputs)[1].detach()
              old_preds = np.argmax(output_base_logits.cpu().numpy(), 1)
              # print(batch[3])
              # print(old_preds)
              weights = (batch[3].detach().cpu().numpy()  == old_preds) * 1 + (batch[3].detach().cpu().numpy()  != old_preds) * .1
              weights = torch.from_numpy(weights).to('cuda:0').float()
              # print(weights)
            outputs = model(**inputs)
            logits = outputs[1]
            
            combined_logits = (logits + output_base_logits)
            # print(torch.nn.functional.cross_entropy(combined_logits, batch[3], reduction = 'none') * weights)
            loss = torch.sum(torch.nn.functional.cross_entropy(combined_logits, batch[3], reduction = 'none') * weights)/torch.sum(weights)
            # loss2 = torch.nn.functional.cross_entropy(output_base_logits, batch[3])
            # if loss.detach().cpu().numpy() > .1:
            #   print(input_ids[0])
            print(loss)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results, _ = evaluate(args, model, tokenizer, model_base, mode="dev")
                ...
                if args.local_rank in [-1, 0] and args.logging_steps == 0 and args.eval_every_epoch:
                    if global_step % (t_total / args.num_train_epochs) == 0:
                        results, _, _ = evaluate(args, model, tokenizer, model_base, mode="dev", prefix="dev.tsv")
...
def evaluate(args, model, tokenizer, model_base, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            logits_base = model_base(**inputs)[1]
            logits = model(**inputs)[1]
            logits = (logits_base + logits)
            tmp_eval_loss = torch.nn.functional.cross_entropy(logits, batch[3], ignore_index=-100)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to pre-trained model to boost off",
    )
    parser.add_argument(
        "--task_name",
        default=None,
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, model_base)
...
#         model.to(args.device)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _, _ = evaluate(args, model, tokenizer, model_base, mode="dev", prefix="dev.tsv")
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(args.output_dir)
        model.to(args.device)
        result, predictions, too_long = evaluate(args, model, tokenizer, model_base, mode="test",
                                       prefix="test.tsv")
        # Save results
