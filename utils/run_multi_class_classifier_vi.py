...
def train(args, train_dataset, model, tokenizer, vi_head):
    """ Train the model """
    ...
    if args.fp16:
        ...
        [model, vi_head], optimizer = amp.initialize([model, vi_head], optimizer, opt_level=args.fp16_opt_level)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            uniform = torch.empty([hidden.shape[0], 1], dtype = hidden.dtype, device = hidden.device).uniform_()
            permutation = torch.randperm(hidden.shape[0])
            hidden_mixup = uniform * hidden[permutation, :] + (1 - uniform) * hidden
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, vi_head)
